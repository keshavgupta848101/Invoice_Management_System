...
class InvoiceForm extends React.Component {
  constructor(props) {
    super(props);
    this.state = {
      queryId: props.location.id ? props.location.id : null,
      redux: props.redux,
      isOpen: false,
      currency: "$",
      currentDate: "",
  ...
  componentDidMount(prevProps) {
    const isAddMode = this.state.queryId;
    const { data, ...reduxState } = this.state.redux;
    const newState = data.filter((state) => state.id == isAddMode);
    ...
    if (this.state.redux.isCopy) {
      console.log(this.state, "copy feature");
      let copyState = this.state.redux.data[this.state.redux.selectedIndex];
      this.setState(
        () => {
          return { ...copyState, id: crypto.randomUUID() };
        },
        () => this.handleCalculateTotal()
      );
    }
  ...
  addValue = () => {
    ...
    const { redux, queryId, ...rest } = this.state;
    const isAddMode = this.state.queryId;
    if (!isAddMode) {
      this.props.dispatch(setInvoiceDetails({ ...rest }));
    } else {
      this.props.dispatch(editInvoiceDetails({ ...rest }));
    }
  };
  render() {
    ...
    return (
      <Container>
        ...
        >
          <BiArrowBack fontSize={"16px"} /> Go Back To Listing Screen
        </div>
                ...
                <Row className="mt-4 justify-content-end">
                  <Col lg={6}>
                    ...
                    >
                      ...
                      <span className="fw-bold">
                        {this.state.currency}
                        {setDecimalPlaces(this.state.total) || 0}
                      </span>
                    </div>
                  </Col>
                </Row>
...
const mapStateToProps = (store) => {
  return {
    redux: store.invoiceManager,
  };
};

export default connect(mapStateToProps)((props) => (
  <InvoiceForm history={useNavigate()} location={useParams()} {...props} />
));
